...
def dijkstra(start, GOAL):
    q = [
        (
            0,
            0,
            (start, (None, None, None, None, None, None, None, None, None, None, None)),
        )
    ]
    hq.heapify(q)
    ...
    while len(q):
        ...
        for p in [0, 1, 3, 5, 7, 9, 10]:
            ...
            # Check if path is obstructed.
            path = (
                hallway[ROOMS[c] : p] if p > ROOMS[c] else hallway[p + 1 : ROOMS[c] + 1]
            )
            if all(z is None for z in path):
                ...
                if all(z in {None, c} for z in room):
                    ...
                    _rooms = tuple(
                        tuple(
                            c if (k == i and l == j) else h for l, h in enumerate(room)
                        )
                        for k, room in enumerate(rooms)
                    )
                    _hallway = tuple(
                        None if k == p else h for k, h in enumerate(hallway)
                    )

                    hq.heappush(q, (_cost, id(_rooms), (_rooms, _hallway)))


rooms = (("D", "C"), ("C", "D"), ("A", "A"), ("B", "B"))
